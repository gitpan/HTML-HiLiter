...
use vars qw( 
		$VERSION $BegChar $EndChar $WordChar $White_Space $HiTag 
...
# if the html we have has no tags (entities okay), call plaintext() instead
	if ( 	$tagless eq $html and
		$tagless !~ m/[<>]/ and
		! $self->{noplain}
		) {
...
sub _make_styles_inline
{
	...
	for (@$queries) {
		...
		my $s = "style='background:" . $colors[$num++] . "'";
		$self->{OTags}->{$_} = "<$tag $s>";
		$self->{CTags}->{$_} = "</$tag>";	# this is always the same; should we bother?
		$num = 0 if $num > $#colors;	# start over if we exceed
						# total number of colors
